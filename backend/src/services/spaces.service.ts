...
import { S3Client, PutObjectCommand, DeleteObjectCommand, DeleteObjectsCommand, GetObjectCommand, ObjectCannedACL } from '@aws-sdk/client-s3';
...
class SpacesService {
   ...
   */
  async uploadImage(file: Express.Multer.File, folder: string = 'uploads'): Promise<UploadedFile> {
    if (!this.s3) {
   ...
   */
  async uploadVideo(file: Express.Multer.File, folder: string = 'videos'): Promise<UploadedFile> {
    if (!this.s3) {
      throw new Error('Spaces not configured. Check DO_SPACES environment variables.');
    }
   ...
   */
  async uploadFile(file: Express.Multer.File, folder: string = 'files'): Promise<UploadedFile> {
    if (!this.s3) {
      throw new Error('Spaces not configured. Check DO_SPACES environment variables.');
    }
