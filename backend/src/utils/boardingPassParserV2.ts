...
import { getAirportInfo } from '../services/airportValidation.service';
...
const PATTERNS = {
  ...
  GATE_TIME_SEAT:
    /(?:GATE\s*)?([A-Z]?\d{1,3}[A-Z]?)\s+(?:BOARDING\s*(?:TILL|TIME)?\s*:?\s*)?(\d{1,2}:\d{2})\s+(?:SEAT\s*)?(\d{1,3}[A-Z])/gi,
};
...
export async function parseBoardingPassV2(buffer: Buffer, mimeType: string): Promise<BoardingPass | null> {
  try {
    ...
    const genericTimeMatches = [
      ...normalizedText.matchAll(PATTERNS.TIME_12H),
      ...normalizedText.matchAll(PATTERNS.TIME_24H),
    ];
    ...
    const boardingInfo: BoardingInfo = {
      ...
      boardingTime:
        boardingTimeMatches.length > 0 ? parseDateTime(parseDate(dateMatches[0]), boardingTimeMatches[0]) : undefined,
...
export function convertToLegacyFormat(boardingPass: BoardingPass): any {
  ...
  const originInfo = getAirportInfo(boardingPass.flight.departure.airportCode);
  const destInfo = getAirportInfo(boardingPass.flight.arrival.airportCode);
  
  const result: any = {
    origin: {
      airportCode: boardingPass.flight.departure.airportCode,
      city: originInfo?.city || boardingPass.flight.departure.city || boardingPass.flight.departure.airportCode,
      country: originInfo?.country || 'USA',
    ...
    destination: {
      ...
      city: destInfo?.city || boardingPass.flight.arrival.city || boardingPass.flight.arrival.airportCode,
      country: destInfo?.country || 'USA',
      name: destInfo?.name,
