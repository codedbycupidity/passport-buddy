...
import { getAirportTimezone, parseZonedDateTime, estimateFlightDuration } from '../services/timeHandling.service';
 ...
 */
export function estimateArrivalTime(departure: Date, origin: string, destination: string): Date {
 ...
 */
export function parseZonedTime(date: Date, timeStr: string, airportCode: string): Date {
 ...
 */
export function correctOcrTimeStrict(timeStr: string, confidence: number): string {
...
export function safeParseTime(
  ...
  try {
    ...
    const timePatterns: Record<string, RegExp[]> = {
      ...
      boarding: [/BOARDING\s*:?\s*(\d{1,2}:\d{2}\s*[AP]M?)/i, /BOARDS?\s*:?\s*(\d{1,2}:\d{2}\s*[AP]M?)/i],
    };
