...
function parseOCRText(text: string): any | null {
  ...
  if (!result.scheduledDepartureTime || !result.scheduledArrivalTime) {
    ...
    const nonBoardingTimes = genericTimes.filter(
      time => !boardingTime || time.toUpperCase() !== boardingTime.toUpperCase()
    );
...
function convertValidationToFlightData(validation: any): any | null {
  ...
  const flightDate = extractedData.date ? extractDate(extractedData.date) : strictDateExtraction();
