...
function extractStructuredData(text: string): ParsedBoardingPass | null {
  try {
    ...
    const tableMatch = text.match(
      /([A-Z0-9]+)\s+(\d{1,2}\s*[A-Z]{3}\s*\d{2,4})\s+(\d{1,2}:\d{2})[^\n]*\n[^\n]*(\d{1,3})\s+(\d{1,2}:\d{2})\s+(\d{1,3}[A-Z])/
    );
    if (tableMatch) {
...
async function extractBoardingPassData(text: string): Promise<ParsedBoardingPass | null> {
  ...
  const departMatch = normalizedText.match(
    /(?:DEPART|DEPARTURE|DEP)\s*:?\s*(\d{1,2}\s*(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[^A-Z]*(?:\d{1,2}:\d{2})?[^A-Z]*)/i
  );
  const arriveMatch = normalizedText.match(
    /(?:ARRIVE|ARRIVAL|ARR)\s*:?\s*(\d{1,2}\s*(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[^A-Z]*(?:\d{1,2}:\d{2})?[^A-Z]*)/i
  );
  ...
  // Boarding group/zone
  let boardingGroup = '';
  const boardingPatterns = [/(?:GROUP|ZONE|BOARDING)\s*:?\s*([A-Z0-9]+)/, /BOARD\s*:?\s*([A-Z0-9]+)/];
