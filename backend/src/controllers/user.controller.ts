...
export const getProfileByUsername = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  const isFollowing = req.userId
    ? user.followers?.some((follower: any) => follower._id.toString() === req.userId)
    : false;
...
export const blockUser = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (!targetUser || !currentUser) {
    console.log(
      `[BLOCK_USER] Failed: User not found. Target user exists: ${!!targetUser}, Current user exists: ${!!currentUser}`
    );
    return next(new AppError('User not found', 404));
  }
  ...
  console.log(
    `[BLOCK_USER] Block operation completed. Current user updated: ${!!currentUserUpdate}, Target user updated: ${!!targetUserUpdate}`
  );
...
export const unblockUser = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  console.log(
    `[UNBLOCK_USER] Attempting to unblock user. Current user: ${currentUserId}, Target user: ${targetUserId}`
  );
  ...
  if (!targetUser || !currentUser) {
    console.log(
      `[UNBLOCK_USER] Failed: User not found. Target user exists: ${!!targetUser}, Current user exists: ${!!currentUser}`
    );
    return next(new AppError('User not found', 404));
  }
...
export const searchUsers = catchAsync(async (req: AuthRequest, res: Response) => {
  ...
  const users = await User.find({
    $or: [{ username: { $regex: searchQuery, $options: 'i' } }, { fullName: { $regex: searchQuery, $options: 'i' } }],
  })
