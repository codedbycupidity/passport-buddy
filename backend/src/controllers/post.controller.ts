...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    const hasMedia =
      (req.uploadedImages && req.uploadedImages.length > 0) || (req.uploadedVideos && req.uploadedVideos.length > 0);
...
export const incrementVideoViews = async (req: Request, res: Response) => {
  try {
    ...
    const updatedPost = await Post.findByIdAndUpdate(id, { $inc: { 'videos.0.views': 1 } }, { new: true });
