...
export const processAndUploadMedia = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const videoPromises = videos.map(async file => {
      ...
      try {
        ...
        try {
          const { thumbnailUrl, metadata } = await videoService.processVideo(file.buffer, file.originalname);
          ...
          console.warn(
            '⚠️ UPLOAD_MIDDLEWARE: Video processing failed, using defaults:',
            (processingError as any).message
          );
    ...
    const [imageResults, videoResults] = await Promise.all([Promise.all(imagePromises), Promise.all(videoPromises)]);
...
export const processAndUploadAvatar = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
