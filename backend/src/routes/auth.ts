...
router.post(
  ...
  catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (existingUser) {
      return next(
        new AppError(existingUser.email === email ? 'Email already registered' : 'Username already taken', 400)
      );
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        createSendToken(newUser, 201, res, 'Registration Successful. Check your email for OTP verification');
      }
    } catch (error) {
