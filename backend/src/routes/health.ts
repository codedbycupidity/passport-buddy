...
router.get('/health', async (_req, res) => {
  ...
  // Overall status
  const hasErrors = Object.values(checks.services).some(status => status === 'DOWN' || status === 'ERROR');
    ...
    if (result) {
      res.json({
        ...
        details: {
          ...
          otp,
          provider:
            process.env.EMAIL_PROVIDER === 'resend' ? 'Resend' : env.MAILTRAP_TOKEN ? 'Mailtrap' : 'Development Mode',
          timestamp: strictDateExtraction(),
        },
      });
    } else {
...
router.get('/email-config', (_req, res) => {
  ...
  const config = {
    configured:
      emailProvider === 'resend' ? !!process.env.RESEND_API_KEY : !!(env.MAILTRAP_TOKEN && env.MAILTRAP_ENDPOINT),
