 ...
 */
export function createZonedDateTime(timestamp: Date | string, airportCode: string): ZonedDateTime {
  // This is implemented in timeHandling.service.ts
  throw new Error('Import from timeHandling.service.ts');
}

export function compareZonedDateTimes(a: ZonedDateTime, b: ZonedDateTime): number {
  // Compares across timezones correctly
  throw new Error('Import from timeHandling.service.ts');
}

export function formatZonedDateTime(zdt: ZonedDateTime, format: string): string {
