...
export async function withRetry<T>(fn: () => Promise<T>, options: RetryOptions = DEFAULT_RETRY_OPTIONS): Promise<T> {
...
export async function waitForDatabase(mongoUri: string, maxWaitTime: number = 30000): Promise<void> {
...
export async function testEmailDelivery(apiUrl: string, testEmail: string): Promise<boolean> {
  try {
...
// Test cleanup helper
export async function cleanupTestData(modelName: string, filter: any): Promise<void> {
  try {
    const mongoose = require('mongoose');
