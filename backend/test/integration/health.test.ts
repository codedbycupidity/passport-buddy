...
describe('API Integration Tests', () => {
  ...
  describe('Authentication', () => {
    ...
    it('should accept request with auth token', async () => {
      const response = await request(app).get('/api/protected').set('Authorization', 'Bearer fake-token').expect(200);
  ...
  describe('Content Type', () => {
    it('should handle JSON requests', async () => {
      ...
      const response = await request(app).post('/api/echo').send(testData).expect('Content-Type', /json/).expect(200);
