...
Only 3 environment files exist in the root directory:

```
Root Directory:
├── .env.dev      - Development configuration (includes DigitalOcean Spaces credentials)
...
2. **Single Source of Truth** - Each environment has one file, no duplicates to maintain

3. **Automatic Environment Selection** - Make commands handle environment switching
