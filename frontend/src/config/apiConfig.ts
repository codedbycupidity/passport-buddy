...
export const getApiConfig = (): ApiConfig => {
  ...
  const isDevelopment = hostname === 'localhost' || hostname === '127.0.0.1' || hostname.endsWith('.local');
  ...
  const productionConfig: ApiConfig = {
    ...
    fallbackUrls: ['https://failover-api.yourservice.com', 'https://emergency-api.yourservice.com'],
    timeout: 10000,
    retries: 3,
  };
...
console.log('🌐 API_CONFIG: Initialized', {
  environment:
    typeof window !== 'undefined' ? (window.location.hostname === 'localhost' ? 'development' : 'production') : 'ssr',
