...
export const useOptimisticLikes = (initialPosts: any[]) => {
  ...
  const toggleLike = useCallback(
    async (postId: string, currentLikes: string[], currentUserId: string) => {
      ...
      try {
        ...
        await requestQueue.add(() => postService.toggleLike(postId), `like-${postId}`);
  ...
  const getLikeState = useCallback(
    (postId: string, originalLikes: string[], currentUserId: string) => {
