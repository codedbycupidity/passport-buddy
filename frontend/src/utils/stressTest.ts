...
export class StressTest {
  ...
  async runConcurrentLikeTest(postId: string, iterations: number = 10) {
    ...
    for (let i = 0; i < iterations; i++) {
      promises.push(this.timedOperation(() => postService.toggleLike(postId), `like-${i}`));
    }
  ...
  async runMixedOperationsTest(postIds: string[], iterations: number = 20) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (operation < 0.4) {
        ...
        // 30% comments
        promises.push(
          this.timedOperation(() => postService.addComment(postId, `Mixed test comment ${i}`), `comment-${i}`)
        );
      } else if (operation < 0.9) {
        // 20% shares
        promises.push(this.timedOperation(() => postService.sharePost(postId), `share-${i}`));
      } else {
        // 10% create posts
        promises.push(
          this.timedOperation(
  ...
  async runRapidFireTest(postId: string, duration: number = 5000) {
    ...
    while (Date.now() - startTime < duration) {
      ...
      for (let i = 0; i < BATCH_SIZE && Date.now() - startTime < duration; i++) {
        batch.push(this.timedOperation(() => postService.toggleLike(postId), `rapid-${operationCount++}`));
      }
  ...
  async runAuthenticationStressTest(iterations: number = 5) {
    ...
    await this.timedOperation(
      () => authService.signup(`stressuser${Date.now()}`, testEmail, testPassword, 'Stress Test User'),
      'register'
    );
    ...
    const promises = [];
    for (let i = 0; i < iterations; i++) {
      promises.push(this.timedOperation(() => authService.login(testEmail, testPassword), `login-${i}`));
    }
  ...
  private async timedOperation(operation: () => Promise<any>, operationName: string): Promise<void> {
  ...
  private generateReport(testName: string): StressTestResults {
    ...
    const totalTime = this.results.operations.reduce((sum, op) => sum + op.duration, 0);
    ...
    console.log(
      `Success Rate: ${((this.results.successfulOperations / this.results.totalOperations) * 100).toFixed(2)}%`
    );
    ...
    const slowestOps = [...this.results.operations].sort((a, b) => b.duration - a.duration).slice(0, 5);
