...
class SmartFetch {
  ...
  async findHealthyEndpoint(skipHealthCheck = false): Promise<string> {
    ...
    const healthChecks = await Promise.allSettled(endpoints.map(endpoint => this.checkBackendHealth(endpoint)));
  ...
  async smartFetch(input: RequestInfo | URL, options: SmartFetchOptions = {}): Promise<Response> {
    const { timeout = API_CONFIG.timeout, skipHealthCheck = false, skipFallback = false, ...fetchOptions } = options;
  ...
  async getEndpointHealth(): Promise<BackendStatus[]> {
    const endpoints = [API_CONFIG.baseUrl, ...API_CONFIG.fallbackUrls];
    const results = await Promise.allSettled(endpoints.map(endpoint => this.checkBackendHealth(endpoint)));
