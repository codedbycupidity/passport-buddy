          ...
          {results.map((airport, index) => (
            ...
            >
              <div className='flex items-center justify-between'>
                <div>
                  <span className='font-semibold text-purple-600'>{highlightMatch(airport.code, query)}</span>
                  <span className='ml-2 text-sm text-gray-600'>{highlightMatch(airport.name, query)}</span>
                </div>
              </div>
              <div className='text-sm text-gray-500'>
                {highlightMatch(
                  `${airport.city}${airport.state ? `, ${airport.state}` : ''}${airport.country ? `, ${airport.country}` : ''}`,
                  query
                )}
              </div>
            </div>
