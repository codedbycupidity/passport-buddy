...
export const CameraFeedback: React.FC<CameraFeedbackProps> = ({ ocrText, onValidation }) => {
  ...
  return (
    <div
      className={`fixed bottom-20 left-4 right-4 p-4 rounded-lg border-2 ${bgColor} transition-all duration-300 ${isValidating ? 'opacity-70' : ''}`}
    >
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center gap-2'>
          ...
          <span className={`font-bold ${textColor}`}>{hasErrors ? 'FIX THESE ISSUES:' : 'LOOKS GOOD!'}</span>
        </div>
        {isValidating && (
        ...
        <ul className='space-y-1'>
          {issues.map((issue, idx) => (
            <li key={idx} className='flex items-start gap-2'>
              <AlertCircle
                className={`w-4 h-4 mt-0.5 ${issue.severity === 'error' ? 'text-red-600' : 'text-yellow-600'}`}
              />
