...
export const AuthErrorModal: React.FC<AuthErrorModalProps> = ({
  ...
  const getRecoveryActions = () => {
    ...
    if (isRecoverable && !isOffline) {
      actions.push(
        <button key='retry' className='auth-modal-retry' onClick={handleRetry} disabled={isRetrying}>
          {isRetrying ? (
            <>
    ...
    actions.push(
      <button key='refresh' className='auth-modal-secondary' onClick={handleRefreshPage}>
    ...
    if ((errorCode === 'UNAUTHORIZED' || errorCode === 'FORBIDDEN') && onGoToLogin) {
      actions.push(
        <button key='login' className='auth-modal-login' onClick={onGoToLogin}>
    ...
    <div className='auth-error-modal-overlay' onClick={onClose}>
      <div className='auth-error-modal' onClick={e => e.stopPropagation()}>
        <div className='auth-error-modal-header'>
          <div className='auth-error-icon'>{getErrorIcon()}</div>

          <h2 className='auth-error-title'>{getErrorTitle()}</h2>

          {onClose && (
            <button className='auth-error-close' onClick={onClose} aria-label='Close'>
              ✕
            </button>
          )}
        </div>
        ...
        <div className='auth-error-modal-body'>
          ...
          <p className='auth-error-help'>{getHelpText()}</p>
          ...
          {/* Actions */}
          <div className='auth-error-actions'>{getRecoveryActions()}</div>

          {/* Advanced diagnostics */}
          <div className='auth-error-advanced'>
            <button className='show-advanced-btn' onClick={() => setShowAdvanced(!showAdvanced)}>
            ...
            {showAdvanced && (
              <div className='advanced-content'>
                <div className='backend-status'>
                  ...
                  <button className='refresh-status-btn' onClick={handleCheckBackend}>
                    🔄 Refresh Status
                  </button>
        ...
        <div className='auth-error-modal-footer'>
          <p>
            Still having issues?{' '}
            <a href='mailto:support@yourservice.com' className='support-link'>
              Contact Support
            </a>
          </p>
        </div>
