...
export function RestFeed() {
  ...
  if (posts.length < 1) {
    return <div className='text-3xl m-8 text-center capitalize font-bold'>No Posts To Show</div>;
  }
      ...
      {posts.map(post => (
        <div key={post._id} className='mt-8'>
          ...
          {post.images && post.images.length > 0 && (
            <div className='mt-2'>
              <img src={post.images[0].url} alt='Post' className='w-full rounded-lg' />
            </div>
          )}
            ...
            <div className='flex items-center space-x-4'>
              <svg
                ...
                className={`cursor-pointer w-6 h-6 ${
                  user?.id && post.likes.includes(user.id) ? 'text-red-500 fill-current' : 'text-gray-700'
                }`}
                fill='none'
                stroke='currentColor'
                viewBox='0 0 24 24'
          ...
          {/* Location */}
          {post.location && <p className='text-sm text-gray-500 mt-1'>📍 {post.location.name}</p>}
          ...
          {post.comments.length > 0 && (
            <div className='mt-2'>
              ...
              {post.comments.length > 2 && (
                <p className='text-sm text-gray-500 mt-1'>View all {post.comments.length} comments</p>
              )}
            </div>
          )}
