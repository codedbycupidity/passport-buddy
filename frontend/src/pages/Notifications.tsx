...
const NotificationItem = memo<NotificationItemProps>(
  ({ notification, onClick, onSenderClick, icon }) => {
    return (
      <div className={`notification-item ${!notification.read ? 'unread' : ''}`} onClick={onClick}>
        <div className='notification-left'>
          {notification.sender?.avatar ? (
            <img src={notification.sender.avatar} alt={notification.sender.username} className='sender-avatar' />
          ) : (
        ...
        <div className='notification-content'>
          <p className='notification-message'>
            <span className='clickable-username' onClick={onSenderClick}>
...
export const Notifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (realtimeNotifications.length > 0) {
      setNotifications(prev => {
        const newNotifs = realtimeNotifications.filter(rtNotif => !prev.some(notif => notif._id === rtNotif._id));
  ...
  const handleMarkAsRead = useCallback(async (id: string) => {
    try {
      ...
      // Revert on error
      setNotifications(prev => prev.map(notif => (notif._id === id ? { ...notif, read: false } : notif)));
    }
  }, []);
